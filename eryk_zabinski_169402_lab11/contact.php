...
if (isset($_POST['show_remind_form'])) {
    ...
    <div id="contact-form-container" style="display: none; margin-top: 20px;">
        ' . PokazKontakt() . '
        <button id="hideContactForm" style="margin-top: 10px;">Ukryj formularz kontaktowy</button>
    </div>
    <script>
        document.getElementById("showContactForm").addEventListener("click", function() {
            document.getElementById("contact-form-container").style.display = "block";
            this.style.display = "none"; // Ukryj przycisk "Pokaż" po kliknięciu
        });

        document.getElementById("contact-form-container").addEventListener("click", function(event) {
            if (event.target.id === "hideContactForm") {
                document.getElementById("contact-form-container").style.display = "none";
                document.getElementById("showContactForm").style.display = "inline-block"; // Pokaż przycisk "Pokaż"
            }
        });
    </script>';
}
